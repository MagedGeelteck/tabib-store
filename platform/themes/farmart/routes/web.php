...
Route::group(['controller' => FarmartController::class, 'middleware' => ['web', 'core']], function () {
  ...
  if((request()->segment(count(request()->segments()))=="en" || request()->segment(count(request()->segments()))=="ar") || request()->segment(count(request()->segments()))==null){
      
    Route::get('/{locale?}', ['uses' => 'index','as' => 'public.index',]);

    }else{
        
    Route::get('/', ['uses' => 'index','as' => 'public.index',]);
    
    }


    if(Request::segment(1)=="en"){
    Route::get('en/catid/{catid}', ['uses' => 'getProductByCategory','as' => 'products.category',]);
    }else{
    Route::get('ar/catid/{catid}', ['uses' => 'getProductByCategory','as' => 'products.category',]);
    Route::get('/catid/{catid}', ['uses' => 'getProductByCategory','as' => 'products.category',]);
    }
