                ...
                <div class="tab-content my-3">
                    <div class="tab-pane fade pt-4 show active" id="nav-login-content" role="tabpanel"
                        ...
                        <form class="mt-3" method="POST" action="{{ route('customer.login.post') }}">
                            @csrf
                            <div class="mb-3">
                                <input class="form-control @if ($errors->has('phone')) is-invalid @endif" type="text" required=""
                                    placeholder="{{ __('Your phone') }}" name="phone"
                            ...
                            @if(Session::get('CodeSent')==true)
                            <div class="input-group mb-3 input-group-with-text">
                                <input class="form-control @if ($errors->has('password')) is-invalid @endif" type="number" placeholder="{{ __('Verification Code') }}"
                                    aria-label="{{ __('Password') }}" autocomplete="current-password" name="otp">
                                
                                @if ($errors->has('Verification Code'))
