                    ...
                    @foreach ($products as $product)
                        ...
                        <li>
                            <div class="product-item product-loop">
                                <img class="lazyload product-item-thumb" src="{{ image_placeholder($thumbUrl) }}" data-src="{{ $thumbUrl }}" alt="{{ $product->name }}" decoding="async" @if(!empty($thumbWidth) && !empty($thumbHeight)) width="{{ $thumbWidth }}" height="{{ $thumbHeight }}" @endif>
                                <h3>{{ $product->name }}</h3>
                                <span class="price">
