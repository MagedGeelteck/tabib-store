...
class LoginController extends Controller
{
    ...
    {
        if($request->otp){
        $find_user=DB::table('ec_customers')->where(['phone'=>$request->phone,'email_verify_token'=>$request->otp])->first();
        if($find_user){
        DB::table('ec_customers')->where(['phone'=>$request->phone,'email_verify_token'=>$request->otp])->update(['confirmed_at'=>Carbon::now()]);
                ...
                throw ValidationException::withMessages([
                    'confirmation' => [
                        __(
                            ...
                            [
                                'resend_link' => route('customer.resend_confirmation', ['phone' => $customer->phone]),
                            ]
                        ),
                    ],
                ]);
