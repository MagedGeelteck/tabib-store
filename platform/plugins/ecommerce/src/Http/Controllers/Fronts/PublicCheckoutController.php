...
class PublicCheckoutController
{
    ...
    public function postCheckout(
        ...
        $verified_number=DB::table('ec_order_addresses')->where(['phone'=>$request['address']['phone'],'verified'=>1])->first();
        
        // if(!$verified_number){
        
        // if(isset($request['SendSmsAgain'])&&$request['SendSmsAgain']==1){
        // $FourDigitRandomNumber = rand(1231,7879);
        // $Number =$request['address']['phone'];
        // $message="Your Order Verification Code is :".$FourDigitRandomNumber;
        ...
        // Session::put('otp',$FourDigitRandomNumber);
        // Session::put('name', $request['address']['name']);
        // Session::put('phone', $request['address']['phone']);
        // Session::put('email', $request['address']['email']??null);
        // Session::put('address', $request['address']['address']);
        // Session::put('description', $request['description']);

        //         return $response
        //         ->setMessage(__('Verification Code Sent successfully!'));
        // }
        // if(isset($request['otp'])&&$request['otp']!=Session::get('otp')){
        //         return $response
        //         ->setError()
        //         ->setMessage(__('Invalid Verification Code !'));
        // }
        ...
        // $verified_number=DB::table('ec_customer_addresses')->where(['id'=>$addressId])->first();
        // $Number=$request['address']['phone']??$verified_number->phone;
        // $message= " تم إستلام طلبك بنجاح";
        // $message.=" يمكنك تتبع حالة الطلب عبر زيارة الرابط التالي";
        // $tracking_code=str_replace('#', '', $order['code']);
        // $message.=" https://tabib-jo.com/orders/tracking?order_id=".$tracking_code;
        // $message = str_replace(' ', '%20', $message);
        // $ch = curl_init();
        // curl_setopt($ch, CURLOPT_URL, "https://www.josms.net/SMSServices/Clients/Prof/RestSingleSMS/SendSMS?senderid=SRV%20Tabib.S&numbers=$Number&accname=tabibstore&AccPass=lN9lL0@hQ4uN@9cP9v&msg=$message");
        //curl_setopt($ch, CURLOPT_URL, "https://josmsservice.com/SMSServices/Clients/Prof/RestSingleSMS/SendSMS?senderid=tabib%20store&numbers=$Number&accname=tabibstore&AccPass=lN9lL0@hQ4uN@9cP9v&msg=$message");
        // curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        // curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "GET");
        // $headers = array();
